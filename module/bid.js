...
exports.mybids = function(req,mysql,q,count)
{
	    ...
	    if(typeof(req.param('cmd')) !== 'undefined')
	    {
	    	...
	    	{
                 var where = ' and b.awarded = 0 and p.date_closed <= "'+datge+'" and (p.market_status != "open" OR p.market_status = "open") and (select count(bb.id) as cnt from bids as bb where bb.project_id = p.id and bb.user_id = b.user_id and bb.awarded = 1) = 0';
	    	}	
	    }	
...
exports.selectProxyAwardBid = function(req,mysql,q)
{
	$mysqli =  {};
	strQuery = mysqli.mysqli($mysqli,'proxy_bid_award');
	...
	query =  mysql.query(strQuery,escape_data,defered.makeNodeResolver());
	//console.log("If proxy bid "+query.sql);
	return defered.promise;
}
...
exports.makeAwardBid = function(req,mysql,q,id)
{
	    ...
	    //console.log(escape_data);               
		query =  mysql.query(strQuery,escape_data,defered.makeNodeResolver());
	console.log("Awarded success "+query.sql);
		return defered.promise;
}
...
exports.allbidders = function(req,mysql,q,pid,uid){

		$mysqli =  {};
		strQuery = mysqli.mysqli($mysqli,815); 
	    
	    var defered = q.defer();
	    
	    var escape_data =  [pid,uid];
	    //console.log(escape_data);               
		query =  mysql.query(strQuery,escape_data,defered.makeNodeResolver());
		return defered.promise;
...
exports.checkUserExistInBidProxy = function(req,mysql,q){

	$mysqli =  {};
	strQuery = mysqli.mysqli($mysqli,"user_in_proxybid");
...
exports.updateProxyBid = function(req,mysql,q){
	...
	$mysqli =  {};
	strQuery = mysqli.mysqli($mysqli,"update_proxybid");

	var defered = q.defer();
	datge = dateFormat(new Date(),"yyyy-mm-dd HH:MM:ss");
	var escape_data =  [req.body.wsprice,datge,req.body.id,req.session.userid];
	//console.log(escape_data);
	query =  mysql.query(strQuery,escape_data,defered.makeNodeResolver());
	return defered.promise;
...
exports.insertProxyBid = function(req,mysql,q){
	...
	$mysqli =  {};
	strQuery = mysqli.mysqli($mysqli,"insert_proxybid");

	var defered = q.defer();
	datge = dateFormat(new Date(),"yyyy-mm-dd HH:MM:ss");
	var escape_data =  [req.body.id,req.session.userid,req.body.wsprice,datge];
	//console.log(escape_data);
	query =  mysql.query(strQuery,escape_data,defered.makeNodeResolver());
	return defered.promise;
}

exports.getProxyDetails = function(req,mysql,q,type){

	$mysqli =  {};
	//mysqli.where = 'WHERE projectORDER BY '
	if(type = 'get_highest_biduser'){
		$mysqli['where'] = ' WHERE project_id = '+req.body.id +' AND status = 1 ORDER BY maxamount DESC LIMIT 1';
	}else if(type = 'second_highest_biduser'){
		$mysqli['where'] = ' WHERE project_id = '+req.body.id +' AND status = 1 ORDER BY maxamount DESC LIMIT 2';
	}
	strQuery = mysqli.mysqli($mysqli,"get_proxy_details");
...
exports.fetch_last_bid = function(req,mysql,q,type){
	...
	$mysqli =  {};
	strQuery = mysqli.mysqli($mysqli,"get_last_bidamount");
...
exports.insertBidsAsProxy = function(req,mysql,q,user_id){
	...
	$mysqli =  {};
	strQuery = mysqli.mysqli($mysqli,"insert_bid_as_proxy");

	var defered = q.defer();
	datge = dateFormat(new Date(),"yyyy-mm-dd HH:MM:ss");
	var escape_data =  [req.body.id,user_id,req.body.next_bid,datge,'bid_as_proxy'];
	//console.log(escape_data);
	query =  mysql.query(strQuery,escape_data,defered.makeNodeResolver());
	return defered.promise;
}