...
mysqli[11] = 'insert into users (email,role,first_name,last_name,password_hash,password_salt,created_at,balance,status,country,state) values ("{{email}}","{{role}}",?,?,"{{password_hash}}","{{password_salt}}","{{created_at}}",0,"moderate","{{country}}","{{state}}")';
...
mysqli[15] = 'select p.*,(select count(distinct b.user_id) as bid from bids as b  where p.id = b.project_id AND b.user_id > 0) as bids from projects AS p where p.user_id = ? and p.market_status !="removed" {{where}} order by id desc limit ?,?';
...
mysqli[32] = 'select b.id,b.proposed_amount,b.user_id,u.first_name,u.email,u.phone,u.last_name,b.project_id from bids as b inner join users as u on u.id = b.user_id where b.project_id = ? order by b.proposed_amount desc limit 1';
...
mysqli[37] = 'select p.title,p.description,b.user_id,b.proposed_amount,date_format(b.awarded_date,"%m/%d/%Y") as adate,b.awarded,p.status,p.avatar,p.date_closed,p.id,(select count(bp.id) as id from bids AS bp where bp.user_id = b.user_id and bp.project_id = p.id) as bids from bids AS b inner join projects as p on p.id = b.project_id and b.user_id = ? {{where}}  group by p.id order by b.id desc {{limit}}';
...
mysqli[75] = 'select c.*, (COUNT(parent.id) - 1) AS depth from categories as c, categories AS parent where c.lft BETWEEN parent.lft AND parent.rgt group by c.id order by c.name asc';
...
mysqli[206] = 'select sum(if(year(p.date_added) = ? and p.date_added>mu.added_date  and p.user_id = ?,1,0)) as ycnt,sum(if(month(p.date_added) = ? and year(p.date_added) = ? and  p.date_added>mu.added_date and p.user_id = ?,1,0)) as mcnt from projects as p left join membership_user as mu on mu.user_id=p.user_id';
...
mysqli[208] = 'update invoices set status = "paid",paid_date = ?,transactionid = ? where primary_id = ? limit 1 ';
...
mysqli['cnx225'] = 'select b.*,p.id as p_id,b.id as b_id,p.title,p.image,p.avatar,p.shipping_description from buynow as b inner join projects as p on b.project_id = p.id where p.user_id = ? and b.qty > 0 order by b.id desc {{limit}}';
mysqli['cnx226'] = 'select * from buynow where user_id = ? and win = 0 and qty > 0';
mysqli['cnx227'] = 'select p.id as id,b.date_added,b.project_id,b.user_id,b.amount,b.ordered,b.cart_id,b.qty,b.refund,b.release,b.r_date,b.feedback,b.paid,b.tracking_number,b.tracking_service,b.win,b.shipping_info,b.escrow,b.commission,b.id as b_id,p.title,p.avatar,p.image,date_format(b.date_added,"%M %D,%Y") as date_add,b.amount as proposed_amount,b.refund as refund_amount,1 as buynow  from buynow as b inner join projects as p on b.project_id = p.id where b.user_id = ? and b.win = 0  and b.qty > 0 order by b.id desc {{limit}}';
mysqli['cnx228'] = 'select * from buynow where user_id = ? and win = 1';
mysqli['cnx229'] = 'select b.*,b.id as b_id,p.title,p.avatar,p.image,date_format(b.date_added,"%M %D,%Y") as date_add from buynow as b inner join projects as p on p.id = b.project_id where b.user_id = ? and b.win = 1 and b.active = 0 order by b.id desc {{limit}}';
...
mysqli[815] = 'SELECT b.id,b.project_id,b.user_id,p.title,u.first_name, u.last_name,u.email,u.phone FROM bids b INNER JOIN projects p ON p.id = b.project_id INNER JOIN users u ON u.id = b.user_id WHERE project_id =? AND b.user_id != ? GROUP BY b.user_id';
...
mysqli['stores_3'] = 'SELECT p2.*,(SELECT pi.avatar from product_images as pi where pi.project_id = p2.id order by pi.id asc limit 0,1) as img1,(SELECT pi.avatar from product_images as pi where pi.project_id = p2.id order by pi.id asc limit 1,1) as img2,(SELECT pi.avatar from product_images as pi where pi.project_id = p2.id order by pi.id asc limit 2,1) as img3  FROM projects  as p2 where p2.id IN (select si.project_id from store_items as si where si.store_id = ? and si.visible = 1) {{where}}'; 
...
mysqli['update_attach_limit'] = 'update users SET used_image_space = used_image_space + ? WHERE id = ?';
...
mysqli[605] = 'select p.*,u.first_name,u.last_name,(select count(distinct b.user_id) as bid from bids as b  where p.id = b.project_id AND b.user_id > 0) as bids from projects AS p inner join users u on u.id= p.user_id where p.market_status !="removed" and DATE(date_closed) >= SYSDATE() and (market_status = "open" or market_status = "moderate") and DATE( date_added ) = DATE( DATE_SUB( SYSDATE( ) , INTERVAL 1 DAY ) ) {{order}} {{limit}}';
mysqli[606] = 'select p.*,u.first_name,u.last_name,(select count(distinct b.user_id) as bid from bids as b  where p.id = b.project_id AND b.user_id > 0) as bids from projects AS p inner join users u on u.id= p.user_id where p.market_status !="removed" and DATE(date_closed) >= SYSDATE() and (market_status = "open" or market_status = "moderate") and DATE( date_added ) BETWEEN DATE( DATE_SUB( SYSDATE( ) , INTERVAL 1 WEEK ) ) AND DATE( SYSDATE( ) )  {{order}} {{limit}}';
mysqli[607] = 'select p.*,u.first_name,u.last_name,(select count(distinct b.user_id) as bid from bids as b  where p.id = b.project_id AND b.user_id > 0) as bids from projects AS p inner join users u on u.id= p.user_id where p.market_status !="removed" and DATE(date_closed) >= SYSDATE() and (market_status = "open" or market_status = "moderate") and DATE( date_added ) BETWEEN DATE( DATE_SUB( SYSDATE( ) , INTERVAL 1 MONTH ) ) AND DATE( SYSDATE( ) )  {{order}} {{limit}}';
mysqli[608] = 'select p.*,u.first_name,u.last_name,(select count(distinct b.user_id) as bid from bids as b  where p.id = b.project_id AND b.user_id > 0) as bids from projects AS p inner join users u on u.id= p.user_id where p.market_status !="removed" and DATE(date_closed) >= SYSDATE() and (market_status = "open" or market_status = "moderate") and DATE( date_added ) BETWEEN DATE( DATE_SUB( SYSDATE( ) , INTERVAL 1 YEAR ) ) AND DATE( SYSDATE( ) )  {{order}} {{limit}}';
mysqli[609] = 'select p.*,u.first_name,u.last_name,(select count(distinct b.user_id) as bid from bids as b  where p.id = b.project_id AND b.user_id > 0) as bids from projects AS p inner join users u on u.id= p.user_id where p.market_status !="removed" and DATE(date_closed) >= SYSDATE() and (market_status = "open" or market_status = "moderate") and DATE( date_added ) BETWEEN DATE( ? ) AND DATE( ? ) {{order}} {{limit}}';
...
mysqli[615] = 'select p.*,b.id as b_id,u.first_name,u.last_name from buynow as b inner join projects as p on b.project_id = p.id left join users u on u.id = b.user_id where b.qty > 0 and DATE( b.paid_date ) = DATE( DATE_SUB( SYSDATE( ) , INTERVAL 1 DAY ) ) {{order}} {{limit}}';
mysqli[616] = 'select p.*,b.id as b_id,u.first_name,u.last_name from buynow as b inner join projects as p on b.project_id = p.id left join users u on u.id = b.user_id where b.qty > 0 and DATE( b.paid_date ) > DATE( DATE_SUB( SYSDATE( ) , INTERVAL 1 WEEK ) ) {{order}} {{limit}}';
mysqli[617] = 'select p.*,b.id as b_id,u.first_name,u.last_name from buynow as b inner join projects as p on b.project_id = p.id left join users u on u.id = b.user_id where b.qty > 0 and DATE( b.paid_date ) > DATE( DATE_SUB( SYSDATE( ) , INTERVAL 1 MONTH ) ) {{order}} {{limit}}';
mysqli[618] = 'select p.*,b.id as b_id,u.first_name,u.last_name from buynow as b inner join projects as p on b.project_id = p.id left join users u on u.id = b.user_id where b.qty > 0 and DATE( b.paid_date ) > DATE( DATE_SUB( SYSDATE( ) , INTERVAL 1 YEAR ) ) {{order}} {{limit}}';
mysqli[619] = 'select p.*,b.id as b_id,u.first_name,u.last_name from buynow as b inner join projects as p on b.project_id = p.id left join users u on u.id = b.user_id where b.qty > 0  and DATE( b.paid_date ) BETWEEN DATE( ? ) AND DATE( ? ) {{order}} {{limit}}';

mysqli[620] = 'select p.*,u.first_name,u.last_name,(select count(distinct b.user_id) as bid from bids as b  where p.id = b.project_id AND b.user_id > 0) as bids from projects AS p inner join users u on u.id= p.user_id where p.market_status !="removed" and DATE(date_closed) >= SYSDATE() and DATE( date_closed ) BETWEEN DATE( SYSDATE( ) ) AND DATE( DATE_ADD( SYSDATE( ) , INTERVAL 1 WEEK ) ) and (market_status = "open" or market_status = "moderate" ) and DATE( date_added ) = DATE( DATE_SUB( SYSDATE( ) , INTERVAL 1 DAY ) )  {{order}} {{limit}}';
mysqli[621] = 'select p.*,u.first_name,u.last_name,(select count(distinct b.user_id) as bid from bids as b  where p.id = b.project_id AND b.user_id > 0) as bids from projects AS p inner join users u on u.id= p.user_id where p.market_status !="removed" and DATE(date_closed) >= SYSDATE() and DATE( date_closed ) BETWEEN DATE( SYSDATE( ) ) AND DATE( DATE_ADD( SYSDATE( ) , INTERVAL 1 WEEK ) ) and (market_status = "open" or market_status = "moderate" ) and DATE( date_added ) BETWEEN DATE( DATE_SUB( SYSDATE( ) , INTERVAL 1 WEEK ) ) AND DATE( SYSDATE( ) )  {{order}} {{limit}}';
mysqli[622] = 'select p.*,u.first_name,u.last_name,(select count(distinct b.user_id) as bid from bids as b  where p.id = b.project_id AND b.user_id > 0) as bids from projects AS p inner join users u on u.id= p.user_id where p.market_status !="removed" and DATE(date_closed) >= SYSDATE() and DATE( date_closed ) BETWEEN DATE( SYSDATE( ) ) AND DATE( DATE_ADD( SYSDATE( ) , INTERVAL 1 WEEK ) ) and (market_status = "open" or market_status = "moderate" ) and DATE( date_added ) BETWEEN DATE( DATE_SUB( SYSDATE( ) , INTERVAL 1 MONTH ) ) AND DATE( SYSDATE( ) )  {{order}} {{limit}}';
mysqli[623] = 'select p.*,u.first_name,u.last_name,(select count(distinct b.user_id) as bid from bids as b  where p.id = b.project_id AND b.user_id > 0) as bids from projects AS p inner join users u on u.id= p.user_id where p.market_status !="removed" and DATE(date_closed) >= SYSDATE() and DATE( date_closed ) BETWEEN DATE( SYSDATE( ) ) AND DATE( DATE_ADD( SYSDATE( ) , INTERVAL 1 WEEK ) ) and (market_status = "open" or market_status = "moderate" ) and DATE( date_added ) BETWEEN DATE( DATE_SUB( SYSDATE( ) , INTERVAL 1 YEAR ) ) AND DATE( SYSDATE( ) )  {{order}} {{limit}}';
mysqli[624] = 'select p.*,u.first_name,u.last_name,(select count(distinct b.user_id) as bid from bids as b  where p.id = b.project_id AND b.user_id > 0) as bids from projects AS p inner join users u on u.id= p.user_id where p.market_status !="removed" and DATE(date_closed) >= SYSDATE() and DATE( date_closed ) BETWEEN DATE( SYSDATE( ) ) AND DATE( DATE_ADD( SYSDATE( ) , INTERVAL 1 WEEK ) ) and (market_status = "open" or market_status = "moderate" ) and DATE( date_added ) BETWEEN DATE( ? ) AND DATE( ? ) {{order}} {{limit}}';
