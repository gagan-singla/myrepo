...
<div class="Listft16"><a href="{$config['url']}/product/view/{$val.id}/{$val.ptitleurl}"  title="{$val.ptitle}"> {$val.title}</a><a href="{$config['url']}/dashboard/product/edit/{$val.id}" {if $val.bids!=0||$val.sold!=0} style="display:none;" {/if} title="{$val.ptitle}">{if $cmd != 'closed'}<span class="red21">Edit</span>{/if}</a></div>
...
<div class="clearfix price">
 ...
 {if $val.auction == 1}
 <div><button class="btn btn-default bid_hbtn" style="margin-top: 3px;"  onclick="bidHistory({$val.id})">Bid History</button> &nbsp; <span style="color:red;font-weight:bold">{{$val.bids}} Bids</span></div>
 <div></div>
 {/if}
</div>
</div>
