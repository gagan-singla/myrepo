...
<div class="ds npayment col-md-9 mobile_view">
  ...
  <div class="panel panel-default">
    <div class=" panel-heading md_hide">
      ...
      {if $payments|count <=  0}
      <div class="row mblue_box1 tno_bids"> No Payment yet </div>
      {else}
    ...
    <div class="panel-body">
      <div class="row">
           ...
           <div data-label="Action" class="account_p_lbl col-md-2">
              {if $val.status == 'paid'}
                 <button class="btn btn-danger" {if $val.type == 'sold'} onclick="window.location='{$config.url}/dashboard/invoice/{$val.buynow_id}'" {/if}  {if $val.type == 'winner'} onclick="window.location='{$config.url}/dashboard/invoice/{$val.buyid}'" {/if}>Paid</button>
              {/if}
