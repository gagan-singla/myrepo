     ...
     <div class="row mblue_box1 tno_bids"> No Won Auction Found </div>
      ...
      {foreach $mybids as $key => $val}
       <div class="row sellp border-btm">
        ...
        <div class="col-md-8">
         <div class="Listft16"><a href="{$config['url']}/product/view/{$val.project_id}/{$val.ptitleurl}"  title="{$val.ptitle}">{$val.title}</a></div>
         ...
         <div class="clearfix price">
          ...
          {if $val.paid == 0}
                ...
                <button type="button" class="btn btn-danger bttns" style="margin-top:5px;" onclick="view_dispute_details({$val.dispute_id},'{$val.subject}')">Show Dispute </button>

                 {/if}
                {if !$val.dispute_id > 0}
                <button type="button" class="btn btn-danger bttns" style="margin-top:5px;" onclick="view_dispute({$val.id})">Add Dispute</button>
                {/if}
