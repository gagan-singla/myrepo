    ...
    <div class=" panel-heading  md_hide">
        ...
        {else}
		 <div class="panel-body">
          ...
          <div  data-label="Image" class="col-md-2 account_p_lbl"><a href="{$config['url']}/product/view/{$val.id}/{$val.ptitleurl}"><img src="{if $val.avatar == ''} {$config['imgpath']}no_img.png {else if $val.avatar != ''}{$config['imgpath']}product/{$val.avatar}{/if}" title="{$val.title}" width="65" height="65"></a></div>
          <div  data-label="Title" class="col-md-4 account_p_lbl"><a href="{$config['url']}/product/view/{$val.id}/{$val.ptitleurl}" title="{$val.ptitle}"> {$val.title}</a></div>
