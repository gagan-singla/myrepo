...
{foreach $proj as $key => $val}
      <div class="slicebox">
        ...
        <div class="slicelft"> <a href="{$config['url']}/product/view/{$val.id}/{$val.ptitleurl}"><img src="{if $val.avatar == ''} {$config['imgpath']}no_img.png {else if $val.avatar != ''}{$config['imgpath']}product/{$val.avatar}{/if}" width="82" title="{$val.title}" height="83" alt="{$val.title}"></a> </div>
        <div class="slicergt">
         <a href="{$config['url']}/product/view/{$val.id}/{$val.ptitleurl}" title="{$val.ptitle}"> <label><a href="{$config['url']}/product/view/{$val.id}/{$val.ptitleurl}" title="{$val.ptitle}"> {$val.ptitle} </label></a>
          <ul>
            <li> Bid to beat: </li>
