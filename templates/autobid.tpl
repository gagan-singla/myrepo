...
<div class="main_container">
  <div class="row dashboard">
    ...
    <div class="row mblue_box1">
      ...
      <div class="col-md-3"><a href="{$config['url']}/product/view/{$val.id}/{$val.ptitleurl}" title="{$val.ptitle}"> {$alist.title}</a></div>
      <div class="col-md-2">{$alist.closedon}</div>
      <div class="col-md-2">{$alist.balance}</div>
      <div class="col-md-2"><a title="View" href="{$config['url']}/product/view/{$alist.id}/{$val.ptitleurl}">View</a></div>
    </div>
    {/foreach}
    {/if}
    </div>
