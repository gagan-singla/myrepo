<div class="row products_row">
  {foreach $proj as $key => $val}
    <div class="col-md-4 col-sm-6">
      <div class="thumbnail" onclick="window.location = '{$config['url']}/product/view/{$val.id}/{$val.ptitleurl}'"  style="height:380px;">
	...
	<div class="row pt_btn">
		 <div class="col-md-8 col-sm-8 col-xs-8">
			{if $val.auction == 0}
			<div class="bid_amount"><span class="prl_lbl">Buynow Price:</span><br /> US $ {$val.bprice}</div> 
			{else if $val.auction == 1}
			<div class="bid_amount"><span class="prl_lbl">Start Price:</span><br /> US $ <span id="price{$val.id}">{$val.wprice}</div>
			{/if}
			</div>
