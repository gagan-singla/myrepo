  ...
  <div class="form-group">
   ...
   {if $val.type == 'radio'}
     {foreach $val.multiple as $key2 => $default}
       <input type="radio" name="questions[{$val.id}]" {if $val.required == 1} required {/if} value="{$default}" {if ($default == $val.default && $val.values == '') || ($val.values == $default)} checked {/if}  />{$default}
      {/foreach} 
   {/if}
   {if $val.type == 'dropdown'}
     <select name="questions[{$val.id}]" {if $val.required == 1} required {/if} class="form-control">
      {foreach $val.multiple as $key2 => $default}
   ...
   {if $val.type == 'checkbox'}
     ...
     {$df2 = $val.values.indexOf($default)}
       <input type="checkbox" name="questions[{$val.id}]" {if $val.required == 1}  required data-parsley-mincheck="1"  {/if} value="{$default}" {if ($default == $val.default && $val.values == '') || ($val.values != '' && $df2 != '-1')} checked {/if} />{$default}
      {/foreach} 
   {/if}
  </div>
</div>
